...
extern esp_ble_mesh_model_t sig_models1[];
extern esp_ble_mesh_model_t sig_models2[];
extern esp_ble_mesh_model_t sig_models3[];

extern esp_ble_mesh_model_t vnd_models[];
extern esp_ble_mesh_model_t vnd_models1[];
extern esp_ble_mesh_model_t vnd_models2[];
extern esp_ble_mesh_model_t vnd_models3[];
...
// rsp opcode sig 
void rd_element_rsp_stt(esp_ble_mesh_model_t *model, uint8_t *par, uint8_t length)  // rsp opcode 0x0482
{
    esp_ble_mesh_msg_ctx_t ctx;
    ctx.net_idx = 0x0000;
    ctx.app_idx = 0x0000;
    ctx.addr = 0x0001; //GW addr
    ctx.send_ttl = ESP_BLE_MESH_TTL_DEFAULT;
    ctx.send_rel = false;  // no ack

    esp_err_t err = esp_ble_mesh_server_model_send_msg(model,
                                                       &ctx, ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_STATUS,
                                                       length, par);
    if (err)
    {
...
void rd_rsp_opcode_E2(esp_ble_mesh_model_cb_param_t *param, uint8_t *par, uint8_t length) // opcode vender
{
...
static void RD_handle_onoff_all(esp_ble_mesh_model_cb_param_t *param)
{
    ...
    if (ele_num == 0xff)
    {
        ...
        for (uint8_t i = 0; i < NUM_ELEMENT; i++)
        {
            ...
            led_set_control(i);
            #if 1
            par_rsp_ele[0] = led_switch[i].current;
            switch (i)
            {
            case 0:
                rd_element_rsp_stt(&root_models, par_rsp_ele, 1);
                break;
            case 1:
                rd_element_rsp_stt(&sig_models1, par_rsp_ele, 1);
                break;
            case 2:
                rd_element_rsp_stt(&sig_models2, par_rsp_ele, 1);
                break;
            case 3:
                rd_element_rsp_stt(&sig_models3, par_rsp_ele, 1);
                break;
            default:
                break;
            }
